...
To run with DDP on 2 nodes, 4 gpus each, example:
...
# note: float16 data type will auto-cast the model to float16 and use a GradScaler for the optimizer
...
if init_from == 'scratch':
    ...
    if dataset == 'shakespeare_char':
        ...
        vocab_size = 50304 # default to GPT-2 vocab size rounded up for efficiency
...
while True:
    ...
    if iter_num % log_interval == 0 and master_process:
        ...
        print(f"iter {iter_num}: loss {lossf:.4f}, time {dt*1000:.2f}ms, lr {lr:.2e}, mfu {running_mfu*100:.2f}%")
    iter_num += 1
    local_iter_num += 1
